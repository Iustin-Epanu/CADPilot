...
from . import svg_section
from . import preview_tools
...
class AgentController:
    ...
    def submit_user_request(
        ...
        # that backend v2 rejects with "no token provided". When running under the
        # local-dev bypass (no _auth_client or _auth_bypass=True), this gate is a
        # no-op and the request proceeds as before.
        token_required = bool(getattr(self, "_auth_client", None)) and not getattr(self, "_auth_bypass", False)
        ...
        payload: Dict[str, Any] = {
            ...
            "timeline_context": feature_tools.list_timeline(self._app),
        }

        # Eyes-on planning (Stage 1 / A4): every execute / planning request
        # carries a fresh viewport screenshot + camera_frame so the planner can
        # ground "the front face" / "the visible hole" against what the user
        # is actually looking at — not just the world axes.
        try:
            screenshot_t0 = time.perf_counter()
            viewport_screenshot_b64 = camera_tools.capture_viewport_screenshot_base64(self._app)
            screenshot_ms = (time.perf_counter() - screenshot_t0) * 1000.0
            if viewport_screenshot_b64:
                payload["viewport_screenshot_base64"] = viewport_screenshot_b64
                logger.info(
                    "[viewport] screenshot captured (%d bytes b64) in %.0fms",
                    len(viewport_screenshot_b64), screenshot_ms,
                )
            else:
                logger.info("[viewport] screenshot capture returned None (%.0fms)", screenshot_ms)
        except Exception as exc:
            logger.warning("[viewport] screenshot capture failed: %s", exc)
        ...
        try:
            ...
            logger.warning("[viewport] camera_frame extraction failed: %s", exc)
        ...
        if "viewport_screenshot_base64" in debug_payload:
            debug_payload["viewport_screenshot_base64"] = (
                f"<base64 omitted, {len(payload['viewport_screenshot_base64'])} chars>"
            )
        if selection_context:
            entities = selection_context.get("entities") or []
    ...
    def _handle_message(self, doc_id: str, message: Dict[str, Any]) -> None:
        ...
        elif message_type == "probe_request":
            self._handle_probe_request(doc_id, message)
        elif message_type == "svg_section_request":
            self._handle_svg_section_request(doc_id, message)
        elif message_type == "blueprint_request":
            self._handle_blueprint_request(doc_id, message)
        elif message_type == "clarification_request":
            self._handle_clarification_request(doc_id, message)
        elif message_type == "request_state_snapshot":
            self._handle_request_state_snapshot(doc_id, message)
        else:
    ...
    def _handle_execute_code(self, doc_id: str, message: Dict[str, Any]) -> None:
        ...
        payload = {
            ...
            # --- NEW: Explicitly pass the reflection data back to FastAPI ---
        ...
        # without dropping the core status keys above. 
    ...
    # ── Tier 4: Agentic Probe ────────────────────────────────────────────────

    def _handle_probe_request(self, doc_id: str, message: Dict[str, Any]) -> None:
        """
        Execute a read-only spatial measurement and return the result to the backend.

        Runs on the Fusion main thread (marshalled via CustomEvent).
        """
        ...
        try:
            result = probe_tools.execute_probe(self._app, tool_name, params)
            if "error" in result:
                success = False
                error_msg = result["error"]
        except Exception as exc:
            logger.exception("[probe] tool=%s failed: %s", tool_name, exc)
            success = False
            error_msg = str(exc)
        ...
        if client:
            try:
                ...
                logger.error("[probe] failed to send result: %s", exc)

    # ── Tier 2: SVG Cross-Section ────────────────────────────────────────────

    def _handle_svg_section_request(self, doc_id: str, message: Dict[str, Any]) -> None:
        """Extract an SVG cross-section and send it back to the backend."""
        probe_id = message.get("probe_id", "")
        plane_axis = message.get("plane_axis", "XY")
        offset_mm = float(message.get("offset_mm", 0.0))

        session_info = self._get_session_info_by_doc(doc_id)
        client = session_info.get("ws_client") if session_info else None

        try:
            result = svg_section.extract_svg_section(self._app, plane_axis, offset_mm)
            payload = {
                "type": "probe_result",
                "probe_id": probe_id,
                "tool": "svg_section",
            ...
            logger.exception("[svg_section] failed: %s", exc)
            payload = {
                "type": "probe_result",
                "probe_id": probe_id,
                "tool": "svg_section",
        ...
        if client:
            try:
                ...
                logger.error("[svg_section] failed to send result: %s", exc)

    # ── Tier 5: Orthographic Blueprinting ────────────────────────────────────

    def _handle_blueprint_request(self, doc_id: str, message: Dict[str, Any]) -> None:
        """Capture orthographic blueprint views and send them to the backend."""
        probe_id = message.get("probe_id", "")
        views = tuple(message.get("views") or ["top", "front", "right"])
        ...
        try:
            result = camera_tools.capture_blueprint_views(self._app, width, height, views)
            payload = {
                "type": "probe_result",
                "probe_id": probe_id,
                "tool": "blueprint_views",
                "result": result,
                "success": result.get("success", False),
            }
        except Exception as exc:
            logger.exception("[blueprint] failed: %s", exc)
            payload = {
                "type": "probe_result",
                "probe_id": probe_id,
                "tool": "blueprint_views",
        ...
        if client:
            try:
                ...
                logger.error("[blueprint] failed to send result: %s", exc)

    # ── Stage 4 / D1: fresh state snapshot for the recovery loop ────────────

    def _handle_request_state_snapshot(self, doc_id: str, message: Dict[str, Any]) -> None:
        """Re-extract entity/timeline/selection/spatial contexts plus a fresh
        viewport screenshot and reply with a `state_snapshot` message.

        Called when the backend's recovery loop needs to see the *current*
        state of the model — step 1 of a script may have created or deleted
        bodies, so the request-time cached snapshot is stale.

        Runs on the Fusion main thread (the WS receive loop already
        marshals through Fusion's custom-event mechanism for this method's
        siblings, e.g. `_handle_probe_request`). Falls back to a partial
        payload on failure so the recovery node still gets something.
        """
        ...
        client = session_info.get("ws_client") if session_info else None
        if not client:
            logger.warning("[state_snapshot] no ws_client for doc_id=%s", doc_id)
            return
        ...
        try:
            payload["entity_context"] = self._extract_entity_context() or {}
        except Exception as exc:
            logger.warning("[state_snapshot] entity_context extraction failed: %s", exc)
            payload["entity_context"] = {}

        try:
            payload["timeline_context"] = feature_tools.list_timeline(self._app)
        except Exception as exc:
            logger.warning("[state_snapshot] timeline extraction failed: %s", exc)
            payload["timeline_context"] = None

        try:
            payload["selection_context"] = extract_selection_context(self._app)
        except Exception as exc:
            logger.debug("[state_snapshot] selection extraction failed: %s", exc)
            payload["selection_context"] = None

        try:
            payload["spatial_context"] = spatial_analyzer.extract_spatial_context(self._app)
        except Exception as exc:
            logger.debug("[state_snapshot] spatial extraction failed: %s", exc)
            payload["spatial_context"] = None

        try:
            shot = camera_tools.capture_viewport_screenshot_base64(self._app)
            if shot:
                payload["viewport_screenshot_base64"] = shot
        except Exception as exc:
            logger.debug("[state_snapshot] screenshot capture failed: %s", exc)

        try:
            cf = camera_tools.get_camera_frame_mm(self._app)
            if cf:
                payload["camera_frame"] = cf
        except Exception as exc:
            logger.debug("[state_snapshot] camera_frame extraction failed: %s", exc)

        try:
            client.send_json(payload)
            logger.info(
                "[state_snapshot] sent fresh snapshot id=%s "
                "(bodies=%d, screenshot=%s)",
                snapshot_id,
                len((payload.get("entity_context") or {}).get("bodies") or []),
                "yes" if "viewport_screenshot_base64" in payload else "no",
            )
        except Exception as exc:
            logger.error("[state_snapshot] send_json failed: %s", exc)

    # ── Stage 3 / C1: clarification dialogue ─────────────────────────────────

    def _handle_clarification_request(self, doc_id: str, message: Dict[str, Any]) -> None:
        """Render a clarification widget by forwarding to the palette UI.

        Each option carries a stable handle (body_handle, entityToken, or
        compound 'body:kind:idx'). For each, we attempt to capture a small
        preview screenshot of the highlighted entity so the widget can show
        the user *which* candidate they're choosing. Heavy operations run
        on the Fusion main thread (this method is invoked from the WS
        receive thread, then delegates to `preview_tools.highlight_entity`
        which is itself main-thread-safe via Fusion's transient API).
        """
        clarification_id = message.get("clarification_id", "")
        question = message.get("question", "")
        rationale = message.get("rationale", "")
        options = message.get("options") or []

        # Best-effort preview enrichment. Skip silently when capture fails
        # so the widget still gets the textual options.
        ...
        for opt in options:
            ...
            if handle and "preview_image_base64" not in entry:
                try:
                    preview = preview_tools.highlight_entity(
                        self._app, handle,
                        capture_screenshot=True,
                        fit=True,
                        restore_selection=True,
                        max_screenshot_width=512,
                    )
                    if preview.get("success") and preview.get("image_base64"):
                        entry["preview_image_base64"] = preview["image_base64"]
                except Exception as exc:
                    logger.debug("[clarification] preview capture failed for %s: %s", handle, exc)
            enriched.append(entry)

        # Forward the dialogue payload to the palette JS for rendering.
        self._palette_manager.send_message(
    ...
    def submit_clarification_response(
        ...
        *,
        ...
        """Send the user's clarification choice back to the backend.

        Called by the palette HTMLEventHandler when the user clicks an
        option (or cancels the dialogue).
        """
        if doc_id is None:
            doc_id = self._active_doc_id
        client = self._get_active_ws_client()
        if not client or not client.is_connected():
            logger.warning(
                "[clarification] cannot submit response: no active WS client"
            )
            return
        ...
        try:
            client.send_json(payload)
            logger.info(
                "[clarification] submitted response: id=%s, option=%s, cancelled=%s",
                clarification_id, selected_option_id, cancelled,
            )
        except Exception as exc:
            logger.error("[clarification] failed to submit response: %s", exc)
