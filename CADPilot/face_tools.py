...
def list_faces(app: adsk.core.Application) -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
    ...
    try:
        for body_index, (ctx, body) in enumerate(iter_brep_bodies(design)):
            ...
            body_handle = compute_body_handle(body_token, body_index)
            ...
            for face_index, face in enumerate(body.faces):
                ...
                # Extract geometry-specific spatial information using objectType dispatch
                try:
                    geometry = face.geometry
                    if geometry:
                        obj_type = geometry.objectType  # e.g. "adsk::core::Plane"

                        if "Plane" in obj_type:
                            try:
                                face_info["origin"] = _point_coords_mm(geometry.origin)
                            except Exception:
                                pass
                            try:
                                geo_normal = geometry.normal
                                face_info["geo_normal"] = [
                                    _rounded(geo_normal.x, 4),
                                    _rounded(geo_normal.y, 4),
                                    _rounded(geo_normal.z, 4),
                                ]
                            except Exception:
                                pass
                            if normal:
                            ...
                            try:
                                ...
                                pass
                            try:
                                face_info["radius"] = _rounded(geometry.radius * 10.0, 6)
                            except Exception:
                                pass
                            try:
                                face_info["origin"] = _point_coords_mm(geometry.origin)
                            ...
                            try:
                                ...
                                pass
                            try:
                                face_info["base_radius"] = _rounded(geometry.baseRadius * 10.0, 6)
                            except Exception:
                                pass
                            try:
                                face_info["half_angle"] = _rounded(geometry.halfAngle, 6)
                            except Exception:
                                pass
                            try:
                                face_info["origin"] = _point_coords_mm(geometry.origin)
                            ...
                            try:
                                face_info["major_radius"] = _rounded(geometry.majorRadius * 10.0, 6)
                            except Exception:
                                pass
                            try:
                                face_info["minor_radius"] = _rounded(geometry.minorRadius * 10.0, 6)
                            except Exception:
                                pass
