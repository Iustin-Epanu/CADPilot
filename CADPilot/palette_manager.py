...
class PaletteManager:
    ...
    def send_message(self, message_type: str, doc_id: Optional[str] = None, **kwargs) -> None:
        ...
        # Append the support contact line onto user-facing error payloads so
        # users always know where to reach us when something fails.
    ...
    def notify(self, args: adsk.core.HTMLEventArgs) -> None:
        ...
        try:
            ...
            if action_name == 'handshake':
                ...
                model_name = payload.get('model_name', 'claude-opus-4-6')
            ...
            elif action_name == 'clarification_response':
                clarification_id = payload.get('clarification_id', '')
                selected_option_id = payload.get('selected_option_id') or payload.get('option_id')
                cancelled = bool(payload.get('cancelled', False))
                handle = payload.get('handle')
                logger.info(
                    "← Clarification response received "
                    f"(id={clarification_id}, option={selected_option_id}, cancelled={cancelled})"
                )
                doc_id = self._controller.get_active_doc_id()
                self._controller.submit_clarification_response(
                    doc_id, clarification_id, selected_option_id,
                    cancelled=cancelled, handle=handle,
                )
