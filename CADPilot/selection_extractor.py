...
def extract_selection_context(app: adsk.core.Application) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        for i in range(active_selections.count):
            ...
            entity_info = _extract_entity_info(entity, app)
            if entity_info:
...
def _extract_entity_info(entity: Any, app: Any = None) -> Optional[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        app: Fusion Application instance; used to resolve the parent body's
             0-based index in ``rootComp.bRepBodies`` so the LLM can write
             ``rootComp.bRepBodies.item(body_index)``.
    ...
    try:
        ...
        if _is_type(object_type, "BRepFace"):
            return _extract_face_info(adsk.fusion.BRepFace.cast(entity), app)

        # Check for BRepEdge
        elif _is_type(object_type, "BRepEdge"):
            return _extract_edge_info(adsk.fusion.BRepEdge.cast(entity), app)

        # Check for BRepBody
        elif _is_type(object_type, "BRepBody"):
            return _extract_body_info(adsk.fusion.BRepBody.cast(entity), app)
...
def _find_body_index(app: Any, body: Optional[adsk.fusion.BRepBody]) -> Optional[int]:
    """
    Return the 0-based index of ``body`` among the root component's bRepBodies,
    matching what `body_tools.list_bodies` emits. Returns None if not found.
    """
    if not body or not app:
        return None
    try:
        ...
        target_token = getattr(body, "entityToken", None)
        for idx in range(root.bRepBodies.count):
            candidate = root.bRepBodies.item(idx)
...
def _attach_parent_body(info: Dict[str, Any], entity: Any, app: Any) -> None:
    """Attach parent body name / index / token / handle to a face- or edge-selection dict."""
    try:
        ...
        if token:
            info["body_token"] = token
        body_idx = _find_body_index(app, body)
        if body_idx is not None:
            info["body_index"] = body_idx
        info["body_handle"] = compute_body_handle(token, body_idx)
    except Exception as exc:
        logger.debug("Failed to attach parent body info: %s", exc)


def _extract_face_info(face: adsk.fusion.BRepFace, app: Any = None) -> Dict[str, Any]:
    """Extract geometry information from a BRepFace."""
    info: Dict[str, Any] = {
        "type": "face"
    }

    _attach_parent_body(info, face, app)

    # Stamp the face's own identifiers so the resolver's selection-fast-path
    # has a concrete handle to bind to. Without these, generated code has no
    # way to address the selected face by token or index.
    try:
        token = getattr(face, "entityToken", None)
        if token:
            info["entity_token"] = token
            info["token"] = token
    except Exception:
        pass
    try:
        ...
        if parent_body is not None:
            for j in range(parent_body.faces.count):
                if parent_body.faces.item(j) is face:
                    info["face_index"] = j
                    break
    except Exception:
        pass
...
def _extract_edge_info(edge: adsk.fusion.BRepEdge, app: Any = None) -> Dict[str, Any]:
    """Extract geometry information from a BRepEdge."""
    info: Dict[str, Any] = {
        "type": "edge"
    }

    _attach_parent_body(info, edge, app)

    # Stamp the edge's own identifiers so generated code can address the
    # selected edge by token / index without re-walking the body.
    try:
        ...
        pass
    try:
        parent_body = getattr(edge, "body", None)
        if parent_body is not None:
            for j in range(parent_body.edges.count):
                if parent_body.edges.item(j) is edge:
                    info["edge_index"] = j
                    break
    except Exception:
        pass
...
def _extract_body_info(body: adsk.fusion.BRepBody, app: Any = None) -> Dict[str, Any]:
    """Extract geometry information from a BRepBody."""
    info: Dict[str, Any] = {
        "type": "body"
    }

    # Resolve body_index so the LLM can call rootComp.bRepBodies.item(body_index).
    body_idx = _find_body_index(app, body)
    if body_idx is not None:
        info["body_index"] = body_idx
    ...
    try:
        ...
        if body_token:
            info["body_token"] = body_token
    except Exception:
        pass
    info["body_handle"] = compute_body_handle(body_token, body_idx)

    try:
        # Get name
        info["name"] = body.name or "Unnamed"
        info["body_name"] = info["name"]
