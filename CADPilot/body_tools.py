...
def compute_body_handle(entity_token: Optional[str], fallback_index: Optional[int] = None) -> str:
    """
    Stable body handle derived from entityToken so the same body produces the
    same handle across snapshots / retries. Falls back to a deterministic
    index-based handle when no token is available.
    """
    if entity_token:
        digest = hashlib.sha1(entity_token.encode("utf-8")).hexdigest()[:8]
        return f"b_{digest}"
    if fallback_index is not None:
        return f"b_idx{int(fallback_index)}"
    return "b_unknown"
...
def list_bodies(app: adsk.core.Application) -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
    ...
    try:
        for body_index, (ctx, body) in enumerate(iter_brep_bodies(design)):
            ...
            body_handle = compute_body_handle(entity_token, body_index)

            body_info: Dict[str, Any] = {
                "id": f"body_{body_index}",  # Updated ID format per spec
                "temp_id": _safe_temp_id(body),
                "name": body_name,
                "token": entity_token,  # Added as 'token' per spec
