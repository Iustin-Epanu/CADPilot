    ...
    try:
        for body_index, (ctx, body) in enumerate(iter_brep_bodies(design)):
            ...
            for edge_index, edge in enumerate(body.edges):
                ...
                # Analytical geometry extraction via objectType
                try:
                    geom = edge.geometry
                    if geom:
                        obj_type = geom.objectType
                        if "Circle3D" in obj_type or "Arc3D" in obj_type:
                            try:
                                edge_info["radius"] = round(geom.radius * 10.0, 6)
                            except Exception:
                                pass
                            try:
                                c = geom.center
                                ...
                                pass
                        elif "Line3D" in obj_type:
                            try:
                                edge_info["geo_length"] = round(geom.length * 10.0, 6)
                            except Exception:
                                pass
                            try:
                                d = geom.direction
                                edge_info["geo_direction"] = [round(d.x, 6), round(d.y, 6), round(d.z, 6)]
                            except Exception:
                                pass
                except Exception as exc:
                    logger.debug("Failed to extract analytical geometry for edge %s: %s", edge_index, exc)
