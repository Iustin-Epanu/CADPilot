...
def list_timeline(app: Optional[adsk.core.Application]) -> List[Dict[str, Any]]:
    """
    Extract timeline history from the active design.

    Iterates design.timeline and returns one dict per active (non-suppressed)
    feature with its type and key defining parameters converted to mm.
    """
    design = _require_design(app)
    timeline = design.timeline
    ...
    for i in range(timeline.count):
        try:
            ...
            obj_type = getattr(entity, "objectType", "") or ""
            feature_info: Dict[str, Any] = {
                "index": i,
                "name": item.name,
                "type": obj_type.split("::")[-1] if "::" in obj_type else obj_type,
            }

            if "ExtrudeFeature" in obj_type:
                try:
                    dist = entity.extentOne.distance
                    feature_info["distance_mm"] = round(float(dist.value) * _CM_TO_MM, 4)
                except Exception:
                    pass

            elif "FilletFeature" in obj_type:
                try:
                    edge_set = entity.edgeSets.item(0)
                    feature_info["radius_mm"] = round(float(edge_set.radius.value) * _CM_TO_MM, 4)
                except Exception:
                    pass
                ...
                try:
                    edge_set = entity.edgeSets.item(0)
                    feature_info["distance_mm"] = round(float(edge_set.distance.value) * _CM_TO_MM, 4)
                except Exception:
                    pass

            elif "RectangularPatternFeature" in obj_type:
                try:
                    feature_info["quantity_one"] = int(entity.quantityOne.value)
                except Exception:
                    pass
                try:
                    feature_info["quantity_two"] = int(entity.quantityTwo.value)
                except Exception:
                    pass
                try:
                    feature_info["distance_one_mm"] = round(float(entity.distanceOne.value) * _CM_TO_MM, 4)
                except Exception:
                    pass

            elif "CircularPatternFeature" in obj_type:
                try:
                    feature_info["quantity"] = int(entity.quantity.value)
                except Exception:
                    pass
                try:
                    feature_info["total_angle_deg"] = round(
                        math.degrees(float(entity.totalAngle.value)), 4
                    )
                except Exception:
                    pass
            ...
            logger.debug("Failed to extract timeline item %d: %s", i, exc)

    return result


# ─────────────────────────────────────────────────────────────────────────────
# Stage 2 / B3 — find_feature_by_phrase
#   Lightweight similarity search over the timeline DNA produced by
#   list_timeline(). Used by the target_resolver node when the user references
#   a prior feature ("the last hole 2mm deeper", "remove the chamfer").
# ─────────────────────────────────────────────────────────────────────────────

# Mapping from common natural-language nouns to substrings expected in
# Fusion's feature objectType (e.g. "ExtrudeFeature", "FilletFeature").
...
_RECENCY_KEYWORDS = {"last", "latest", "recent", "newest", "most"}
_OLDEST_KEYWORDS = {"first", "earliest", "original", "oldest"}


def _phrase_tokens_ft(phrase: str) -> List[str]:
    return [t for t in (phrase or "").lower().replace("-", " ").split() if t]


def find_feature_by_phrase(
    timeline: List[Dict[str, Any]],
    phrase: str,
    *,
    top_n: int = 4,
) -> List[Dict[str, Any]]:
    """Score timeline entries against a natural-language phrase.

    Strategy:
      - Type-match: phrase word maps to feature type substring (e.g. "hole" →
        "Hole", "round" → "Fillet"). Heavy weight (0.6).
      - Name-match: phrase words found in the feature's user-given name.
        Per-token weight (0.15 each, capped).
      - Recency: "last" / "latest" boosts the most recent matching feature.
        "first" / "earliest" boosts the oldest. Weight (0.25).

    Returns the top ``top_n`` matches sorted by score descending. Each entry
    is the original timeline dict augmented with ``"score"`` and ``"rationale"``.
    Returns ``[]`` when no entry scores above zero — callers (resolver) treat
    that as "ask the user".
    """
    if not timeline or not phrase:
        return []

    tokens = _phrase_tokens_ft(phrase)
    token_set = set(tokens)

    type_targets: List[str] = []
    for tok in tokens:
        if tok in _FEATURE_TYPE_KEYWORDS:
            type_targets.extend(_FEATURE_TYPE_KEYWORDS[tok])
    type_targets_set = set(type_targets)

    wants_recent = bool(token_set & _RECENCY_KEYWORDS)
    wants_oldest = bool(token_set & _OLDEST_KEYWORDS)

    # Find max index for normalising recency.
    max_index = max((int(item.get("index", 0) or 0) for item in timeline), default=0)
    max_index = max(max_index, 1)  # avoid div-by-zero
    ...
    for item in timeline:
        ...
        # Type match.
        if type_targets_set:
            for target in type_targets_set:
                if target in ftype:
                    score += 0.6
                    rationales.append(f"type='{ftype}' matches '{target.lower()}'")
                    break

        # Name overlap.
        if fname:
            name_toks = set(_phrase_tokens_ft(fname))
            overlap = token_set & name_toks
            # Filter out generic words that always overlap.
            overlap -= {"the", "a", "of", "in", "on", "and"}
            if overlap:
                bump = min(0.45, 0.15 * len(overlap))
                score += bump
                rationales.append(f"name overlaps {sorted(overlap)}")

        # Recency.
        if wants_recent and score > 0:
            recency = idx / max_index
            score += 0.25 * recency
            if recency > 0.7:
                rationales.append("recent (matches 'last/latest')")
        if wants_oldest and score > 0:
            oldness = 1.0 - (idx / max_index)
            score += 0.25 * oldness
            if oldness > 0.7:
                rationales.append("early (matches 'first/earliest')")

        if score <= 0:
            continue

        entry = dict(item)
        # Token field is reserved for a future stage that surfaces feature
        # entityTokens through list_timeline; keep the slot present so the
        # resolver's downstream code can detect missing-token cases.
        entry.setdefault("token", None)
        entry["score"] = round(min(score, 1.0), 4)
        entry["rationale"] = "; ".join(rationales) if rationales else "weak match"
        scored.append(entry)
